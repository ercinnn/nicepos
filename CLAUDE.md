...
**Kalan (Faz F Adım 2 + sonrası, henüz yapılmadı):** kendi domain bağlama (tenant'ın satın aldığı domain'i CNAME ile Cloudflare Pages'e yönlendirip host→tenant eşlemesi — Cloudflare API token'ı, DNS doğrulama akışı, SSL provisioning gerektirir, Adım 1'den bağımsız ayrı bir mühendislik turu), gerçek faturalandırma/ödeme (iyzico entegrasyonu bekliyor).
