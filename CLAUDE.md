...
- **⚠️ Cloudflare endpoint yolları EN İYİ TAHMİN:** Registrar API beta olduğundan ve tam dokümantasyona bu turda erişilemediğinden (`/accounts/{account_id}/registrar/...` deseni varsayıldı) Aşama B'de gerçek hesapla test edilirken `_shared/cloudflare.ts`'in `RealCloudflareClient` sınıfı https://developers.cloudflare.com/registrar/registrar-api/ 'a karşı doğrulanıp gerekirse düzeltilmeli.
...
- **Her kullanıcı için domain adresi alma sekmesi** — Faz F Adım 1 (bkz. "Online Satış (Storefront)") ile kiracı artık paylaşılan `nicepos-online-satis.pages.dev`'i `?magaza=<slug>` ile kendi mağazası olarak kullanabiliyor. Kalan (Adım 2): kendi SATIN ALDIĞI domain'i bağlayabilme — Cloudflare API token'ı + DNS doğrulama akışı + SSL provisioning gerektiren ayrı bir mühendislik turu, henüz başlanmadı.
