...
**AI ile Grupla** (toolbar butonu, yalnız masaüstü): `group_id` boş ürünlere ürün adına göre otomatik üst grup+grup önerir — **harici bir LLM/API kullanmaz** (kullanıcı isteği: "kendi makine öğrenmemiz"), tamamen sunucu tarafında pg_trgm tabanlı bir k-En Yakın Komşu (k-NN) sınıflandırıcı (`suggest_product_groups` RPC, `0063_product_group_suggestions.sql`). Yöntem: her etiketsiz ürün adı için GiST trigram indeksi (`idx_products_name_trgm_gist`) üzerinden `<->` KNN operatörüyle en benzer p_k (varsayılan 5) ETİKETLİ (zaten gruplu) ürün bulunur, bunların gruplarına benzerlik-ağırlıklı oy verilir; confidence = kazanan grubu destekleyen komşuların ORTALAMA benzerliği (bkz. aşağıdaki yaşanmış hata). Öneri HER ZAMAN mevcut bir gruba atar, yeni grup uydurmaz — yeterince benzer komşu yoksa "sınıflandırılamadı" listesine düşer. UI (`ai_grouping_dialog.dart`): öneriler önce gösterilir (güven rozetli, <%50 varsayılan işaretsiz, satır bazlı manuel grup değişimi mümkün), kullanıcı onaylayınca `ProductRepository.bulkAssignGroups()` ile toplu (aynı gruba gidenler `inFilter` + 100'lük chunk'larla tek sorguda) uygulanır.
- **Kapsam = ekrandaki filtreye uyan ürünler (kullanıcı kararı):** buton tıklanınca `products_list_screen.dart` Excel Aktar ile AYNI `fetchAll(query:, groupId:, filters:, sortColumn:, sortAscending:, activeOnly:)` çağrısıyla o anki arama/sütun filtrelerine uyan TÜM ürünleri (sayfalamadan bağımsız) çeker, `group_id == null` olanların id'sini `AiGroupingDialog(candidateProductIds:)`'e geçirir. RPC hâlâ TÜM etiketsiz ürünler için hesaplar (SQL değişmedi) — dialog dönen öneriler/"sınıflandırılamadı" listesini bu id kümesiyle **istemci tarafında** kesişime sokar; filtre dışı ürünler ne önerilir ne de "sınıflandırılamadı" sayılır. `candidateProductIds` boşsa (filtreye uyan grubu-boş ürün yok) ayrı bir boş-durum mesajı gösterilir.
- **⚠️ MATERIALIZED CTE tuzağı:** `labeled`/`matches` CTE'lerini `MATERIALIZED` yapmak (CLAUDE.md'nin genel "büyük tabloya JOIN + window fonksiyonu" tavsiyesinin AKSİNE) trigram indeksinin join'de kullanılmasını engelleyip performansı ÇÖKERTTİ — hiçbir CTE birden fazla kez referans alınmadığından zaten gereksizdi.
- **⚠️ GIN yerine GiST + KNN:** ilk sürüm eşik-bazlı `%` operatörü + GIN indeksiyle SQL Editor'da (postgres/süper kullanıcı, RLS yok) hızlı görünüyordu ama uygulamadan `authenticated` rolüyle (RLS aktif) çağrılınca `57014` (statement timeout) verdi — `ORDER BY name <-> :query LIMIT p_k` + GiST (`gist_trgm_ops`) klasik top-k KNN desenine geçildi.
- **⚠️ plpgsql fonksiyonu EXPLAIN'e kara kutu görünür:** `explain analyze select * from suggest_product_groups(...)` içindeki sorgunun planını GÖSTERMEZ (tek "Function Scan" düğümü) — gerçek index kullanımını görmek için CTE zincirini fonksiyon DIŞINDA, düz bir sorgu olarak `explain (analyze, buffers)` ile çalıştırmak gerekir.
- **⚠️ Fonksiyona özel `statement_timeout`:** GiST KNN indeksi kullanılsa bile trigram imzalarının doğası gereği binlerce üründe toplam ~15-20sn sürebiliyor (ölçülen: 3242 üründe ~18.7sn) — bu, Supabase'in `authenticated` rolü için varsayılan API zaman aşımını aşıyordu. Global rol timeout'u değiştirmek yerine `alter function suggest_product_groups(int,numeric) set statement_timeout='60s'` ile YALNIZ bu fonksiyona özel yükseltildi (sık tetiklenmeyen, kullanıcının bilinçli başlattığı bir toplu işlem olduğundan ~15-20sn bekleme + dialogdaki spinner kabul edilebilir).
- **⚠️ confidence = oy PAYI DEĞİL, ortalama benzerlik:** "kazanan grubun rakiplere göre oy payı" hesaplamak, rakip grup yoksa (çoğu durumda böyle) benzerlik skorundan bağımsız hep %100 gösteriyordu (canlı veride tek %35 benzerlikli eşleşme bile "%100 güven" görünüyordu) — confidence artık `avg(sim)` (kazanan grubu destekleyen komşuların ortalama benzerliği).
...
**⚠️⚠️ "Freshen edip devam et" tavsiyesi kabuksuz agent DURDUĞUNDA işe yaramayabilir (yaşanmış, storefront v1.1 turu):** Agent Adım 0'da tabanı bayat bulup hiçbir dosyaya dokunmadan durursa, Agent tool'u worktree'yi **otomatik siler** ("worktree is automatically cleaned up if the agent makes no changes") — `git worktree list` boş döner, freshen edilecek bir worktree KALMAZ. Üstelik bu yalnız "art arda ikinci/üçüncü görev" değil; AYNI oturumda ana checkout'ta commit'ler yapılmış olsa bile **yeni açılan worktree oturumun BAŞLADIĞI commit'ten** açılabiliyor (gözlemlenen: tasarım lideri'nin token commit'i + bağımlılık commit'i sonrasında bile art arda iki `magaza-tasarimci` çağrısı aynı bayat tabandan açıldı). Pratik çözüm: kabuksuz bir agent (`Read/Edit/Glob/Grep`, `Bash` YOK) bu döngüye girerse ve o an başka hiçbir arka plan agent'ı aynı dosyalarla çalışmıyorsa, **`isolation` parametresini tamamen ATLA** — agent doğrudan ana checkout'ta düzenler (worktree/merge dansı yok), ana oturum diff'i gözden geçirip kendisi commit eder. Yalnız birden fazla agent PARALEL çalışıyorsa (izolasyon gerçekten gerekliyse) bu kısayol uygun değildir.
...
- **e-Fatura/e-Arşiv entegrasyonu (EDM Bilişim)** — ayrıntılı plan `notes/e-fatura-entegrasyonu.md`'de. DB şeması (`tenants` fatura alanları + `sale_items` KDV donması + `sale_invoices` talep kuyruğu, migration 0064-0066) + `SaleEditScreen`'deki "Fatura Kes" butonu + kökte Python iskeleti (`efatura/`, elle çalıştırılan CLI) hazır; yalnız EDM API bilgisi (kullanıcı adı/şifre, endpoint, format) geldiğinde `efatura/edm_client.py` doldurulacak. Migration'lar henüz Supabase SQL Editor'da uygulanmadı.
