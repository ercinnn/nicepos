...
- **Audit log kapsamının genişletilmesi** — `lib/features/audit/` (0044 migration, `audit_log` tablosu) v1'de yalnız "Satışı Sil"i loglar (`AuditLogRepository.log`, `SaleEditScreen._delete()`'ten çağrılır). Ürün silme, müşteri/borç hareketi silme gibi diğer kalıcı işlemler de aynı tabloya (`action`/`entity_type` serbest metin) eklenebilir — henüz eklenmedi.
