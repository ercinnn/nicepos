...
- Rol-bazlı UI kısıtlaması (staff bazı ekranları görmesin) BİLİNÇLİ OLARAK henüz YOK — `currentMembershipProvider.role` mevcut ama hiçbir ekranı kısıtlamıyor, hangi ekranların staff'a kapalı olacağı kullanıcı kararı bekliyor.
...
**Kalan fazlar (henüz yapılmadı):** Faz D (Geniş Logo/İndirim Etiketi'ndeki sabit NicePOS logolarının kiracı-bazlı hale getirilmesi — Storage artık izole olduğundan önü açık), Faz F (storefront çoklu-kiracı, slug/subdomain yönlendirmesi), rol-bazlı ekran kısıtlaması, faturalama.
...
- **Rol-bazlı ekran kısıtlaması** — `memberships.role` (owner/admin/staff) altyapısı zaten var (bkz. "Çok Kiracılı Mimari" bölümü, Faz A/B) ve personel davet akışı çalışıyor; eksik olan yalnız UI tarafı — hangi ekranların (Kasa? maliyet fiyatları? Satışı Sil?) staff'a kapalı olacağına kullanıcı karar vermeli, sonra `currentMembershipProvider.role` ile gate'lenir. Bu eklenince "Satışı Sil" gibi kalıcı/geri alınamaz işlemler için bir audit log (kim ne zaman ne sildi) da doğal bir uzantı olur.
