-- =============================================================================
-- 0018: search_products RPC'sinde "Durum" filtresi zaman aşımı düzeltmesi
-- =============================================================================
-- 0017_search_products_rpc.sql, `product_status` view'ını (bkz.
-- 0016_product_status.sql) TÜM `products` tablosuna karşı LEFT JOIN ediyordu.
-- O view'ın içindeki `weekly_sales`/`cok_satan` CTE'leri (son 28 günün
-- `sale_items`/`sales` agregasyonu) PostgreSQL 12+'te varsayılan olarak
-- INLINE edilir (MATERIALIZED değil) — büyük bir JOIN içine gömülünce planlayıcı
-- bu agregasyonu satır başına (correlated) yeniden hesaplayan bir plan
-- seçebiliyor. Sonuç: "Tükendi" gibi filtrelerde sorgu zaman aşımına uğrayıp
-- "Canceling statement due to statement timeout (57014)" hatası veriyordu
-- (canlıda gözlemlendi) — halbuki AYNI hesaplama `product_status` view'ı TEK
-- BAŞINA (küçük id kümesiyle, bkz. fetchStatuses) sorgulandığında sorunsuz
-- çalışıyor.
--
-- Çözüm: view'a JOIN etmek yerine aynı mantık RPC içine kopyalanır, ama
-- `weekly_sales`/`cok_satan` CTE'leri AÇIKÇA `MATERIALIZED` işaretlenir —
-- Postgres'i bu agregasyonu TEK SEFERDE hesaplayıp sonucu geçici bir tabloya
-- alması, sonra `products`'a ucuz bir hash/index join ile bağlanması için
-- zorlar (satır başına yeniden hesaplama riski ortadan kalkar).
--
-- Öncelik sırası (view ile BİREBİR aynı, bkz. 0016): Çok Satan > Tükendi >
-- Pasif > (boş).
--
...
as $$
  with weekly_sales as materialized (
    ...
    where si.product_id is not null
      ...
      and s.sale_date < now()
      -- Yalnız durum filtresi aktifken bu agregasyona gerek var; boş sorguda
      -- bile PostgreSQL 12+ `not p_status is null` sabit koşulunu erken
      -- eleyip bu CTE'yi atlayabilir (materialized olsa da planlayıcı hâlâ
      -- gereksiz taramayı budayabilir) — güvenlik için WHERE'de yine de
      -- p_status kontrolü var.
    group by 1, 2
  ),
