-- =============================================================================
-- 0019: search_products RPC'sine sıralama + barkod birebir eşleşme
-- =============================================================================
-- İki değişiklik (kullanıcı isteği):
-- 1. Barkod sütun filtresi eskiden "içerir" (ilike %x%) idi — "002" yazınca
--    içinde 002 GEÇEN tüm barkodlar geliyordu. Artık BİREBİR eşleşme (=).
-- 2. Ürünler tablosunda sütun başlığına tıklayınca sıralama: Ürün Adı (A-Z/
--    Z-A) ve sayısal sütunlar (Barkod, Stok, Kritik Stok, KDV, Alış Fiyatı,
--    Fiyat 1, Fiyat 2) büyükten küçüğe/küçükten büyüğe. `language sql`
--    fonksiyonda sütun adı doğrudan parametre olarak ORDER BY'a konulamaz
--    (dinamik SQL/EXECUTE gerektirir, injection riski taşır) — bunun yerine
--    her sıralanabilir sütun için bir ASC + bir DESC CASE çifti yazılır;
--    `p_sort_column`/`p_sort_ascending`'e göre yalnız BİR çift her satırda
--    non-null olur, diğerleri null (etkisiz) kalır. Beyaz liste zaten Dart
--    tarafında da uygulanıyor (bkz. ProductRepository.sortableColumns) —
--    burası ikinci savunma katmanı.
--
