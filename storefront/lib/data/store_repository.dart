...
class StoreRepository {
  ...
  Future<List<StoreProduct>> fetchProducts({String? query, String? groupId}) async {
    ...
    return (rows as List).map((row) => StoreProduct.fromMap(Map<String, dynamic>.from(row as Map))).toList();
  }

  Future<StoreProduct?> fetchProductById(String id) async {
    final row = await _client.from('online_products').select().eq('id', id).maybeSingle();
  ...
  Future<List<StoreCategory>> fetchCategories() async {
    final rows = await _client.from('product_groups').select('id, name, parent_group_id').order('name');
    return (rows as List).map((row) => StoreCategory.fromMap(Map<String, dynamic>.from(row as Map))).toList();
  }
  ...
  Future<String> createOrder({
    ...
    final result = await _client.rpc('create_online_order', params: {
