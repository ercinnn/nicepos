...
/// Birden fazla satışı TEK bir A4 dokümanında (her satış kendi bölümüyle,
/// art arda) yeni bir tarayıcı penceresinde açar ve otomatik yazdırır.
/// Müşteriye geçmiş alışverişlerinin toplu dökümünü tek seferde vermek için.
void printMultipleSalesA4({
  required String customerName,
  required List<Sale> sales,
  required Map<String, List<SaleItem>> itemsBySaleId,
}) {
  final html = _buildMultiHtml(
    customerName: customerName,
    sales: sales,
...
String _buildMultiHtml({
  ...
  for (final s in sales) {
    final items = itemsBySaleId[s.id] ?? const <SaleItem>[];
    final subtotal = items.fold<num>(0, (sum, it) => sum + it.total);
    ...
    final discountRow = s.discountAmount > 0
        ? '<tr><td>İskonto</td><td class="r">- ${_esc(formatCurrency(s.discountAmount))}</td></tr>'
        : '';

    blocks.writeln('''
      <div class="sale-block">
        <div class="sale-head">
          <div><b>Belge No:</b> ${_esc(s.saleCode)} &nbsp; <b>Tarih:</b> ${_esc(formatDateTime(s.saleDate))} &nbsp; <b>Ödeme:</b> ${_esc(s.paymentType.label)}</div>
        </div>
        <table>
          <thead>
            <tr>
        ...
        <div class="totals">
          <table>
            <tr><td>Ara Toplam</td><td class="r">${_esc(formatCurrency(subtotal))}</td></tr>
            $discountRow
            <tr class="net"><td>Genel Toplam</td><td class="r">${_esc(formatCurrency(s.totalAmount))}</td></tr>
          </table>
        </div>
      </div>''');
  }
...
<head>
<meta charset="utf-8">
<title>Geçmiş Alışverişler — ${_esc(customerName)}</title>
<style>
  @page { size: A4 portrait; margin: 16mm; }
  * { box-sizing: border-box; }
  body {
  ...
  .head .meta { font-size: 11px; color: #555; line-height: 1.6; }
  .head .meta b { color: #1a2233; }
  .sale-block { margin-bottom: 22px; page-break-inside: avoid; }
  .sale-head {
    background: #FDF6E3;
  ...
  td.c { text-align: center; color: #888; }
  td.bc { font-family: "Courier New", monospace; color: #444; white-space: nowrap; }
  .totals { margin-top: 6px; width: 280px; margin-left: auto; }
  .totals table { width: 100%; }
...
<body onload="window.focus(); window.print();">
  <div class="head">
    <h1>Geçmiş Alışverişler</h1>
    <div class="meta">
      <div>Müşteri: <b>${_esc(customerName)}</b></div>
      <div>${sales.length} satış — yazdırma tarihi: <b>${_esc(formatDateTime(DateTime.now()))}</b></div>
    </div>
  </div>

  $blocks

  <div class="grand">Tüm Seçili Satışlar Toplamı: ${_esc(formatCurrency(grandTotal))}</div>
