...
// ═══════════════════════════════════════════════════════════════════════════
// Geniş Logo etiketi PDF üretimi — 2 sütun × 5 satır = 10 etiket. Kesin baskı
// geometrisi (önizleme = HTML = PDF, üçü BİREBİR):
//   • A4 dikey 210×297mm · hücre 88×55mm · kenar üst/alt 11mm, sol/sağ 17mm
//     → 10 etiket A4'e tam ortalı.
//   • Faz D: sabit marka illüstrasyonu KALDIRILDI — kiracının kendi logosu
//     (opsiyonel, RENKLİ) düz zeminde ortalı; dikey `Expanded(flex:)` akışı:
//     logo → fiyat → ürün adı (2 satır) → Code128 → alt satır (barkod no +
//     tarih). Herhangi bir logonun en-boy oranıyla çalışır.
// ═══════════════════════════════════════════════════════════════════════════
...
// Ürün adı boyutu (Faz D redesign — flex-oran gövde, sabit pixel-offset yerine
// `Expanded(flex:)`; Tel İndirim'in aynı dersi paylaşır, bkz. CLAUDE.md).
...
/// bazlı marka, `LabelSheetState.logoDataUrl` ile PAYLAŞILIR); yoksa mağaza
/// ikonu fallback kullanılır (Raf/Poster ile aynı desen).
Future<Uint8List> buildWideLabelsPdf({
  required List<LabelSlot?> slots,
  String? logoDataUrl,
}) =>
    buildWideLabelsPdfMultiPage(pages: [slots], logoDataUrl: logoDataUrl);
...
/// Tema/logo kurulumu PDF başına BİR KEZ yapılır; mevcut tek-sayfalı
              ...
              return pw.Expanded(
                child: pw.Row(
                  children: List.generate(_kWideCols, (c) {
                    ...
                    final slot = idx < slots.length ? slots[idx] : null;
                    return pw.Expanded(child: _wideCell(slot, logoImage));
                  }),
                ),
              );
            }),
...
// fiyat/ad/barkod, `Expanded(flex:)` oranlı dikey akış; ARTIK sabit marka
// illüstrasyonu arka plan ÜZERİNE bindirme YOK — herhangi bir logonun en-boy
// oranıyla çalışır, KARAR: paylaşılan logo mekanizması). Boş hane → yalnız
// ince kesim kılavuzu.
pw.Widget _wideCell(LabelSlot? slot, pw.MemoryImage? logoImage) {
  if (slot == null) {
    return pw.Container(
      decoration: pw.BoxDecoration(
        border: pw.Border.all(color: _hairlineEmpty, width: 0.5),
  ...
  return pw.Container(
    ...
    child: pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.center,
      children: [
        // Logo — ortalı, düz zemin (kiracı yüklemediyse boş, sabit marka
        // varsayılanı YOK — Faz D "yeni kiracılar nötr başlar" ilkesi).
        pw.Expanded(
          flex: 32,
          child: pw.Center(
            child: logoImage != null
        ...
        // Ürün adı (ortalı, en çok 2 satır, taşarsa kırpar).
        pw.Expanded(
          flex: 20,
          child: pw.ClipRect(
            child: pw.Align(
...
// (kullanıcı isteği, çok sayfalı YOK). Logo + tagline artık kiracı-bazlı
// (Faz D — `LabelSheetState.logoDataUrl`/`taglineText`, Geniş Logo ile
// PAYLAŞILIR, çağıran taraf geçirir); sabit marka figürü/metni KALDIRILDI.
// Hücre-içi (kullanıcı referans mockup'ına göre, KARAR): logo + tagline
// (siyah) + ince ayraç + ürün adı (BÜYÜK HARF) + tek satır kırmızı "%X
// İNDİRİM" bandı + "ESKİ FİYAT: " (siyah, üzeri KIRMIZI çizili) + kutulu
...
/// girmemişse (`discountPercent == null`) bu değer kullanılır. [logoDataUrl]/
/// [tagline] — Faz D kiracı-bazlı marka (Geniş Logo ile PAYLAŞILIR).
Future<Uint8List> buildDiscountLabelsPdf({
  ...
  String tagline = '',
    ...
    buildDiscountLabelsPdfMultiPage(
      pages: paginateDiscountSlots(slots),
              ...
              return pw.Expanded(
                child: pw.Row(
                  children: List.generate(_kDiscountCols, (c) {
                    ...
                    return pw.Expanded(
                        child: _discountCell(
                            slot, logoImage, defaultPercent, tagline));
                  }),
                ),
              );
            }),
...
pw.Widget _discountCell(DiscountLabelSlot? slot, pw.MemoryImage? logoImage,
  ...
  return pw.Container(
    ...
    child: pw.Column(
      ...
      mainAxisAlignment: pw.MainAxisAlignment.start,
      children: [
        // Logo — hane başı `showLogo` tikliyse kiracının kendi logosu (Faz D,
        // Geniş Logo ile PAYLAŞILIR); tiksizse VEYA logo yüklenmediyse alan
        // BOŞ (yükseklik korunur, yalnız görsel basılmaz).
        pw.SizedBox(
          height: 68,
          child: (slot.showLogo && logoImage != null)
              ? pw.Image(logoImage, fit: pw.BoxFit.contain)
              : pw.SizedBox(),
        ),
